...
class BaseMover(object):
    ...
    def __init__(self, root, canvas, position, x, y, tags, w, h, play):
        ...
        self.canvas = canvas
        # 移动者的状态
        self.states = config.life_stauts_alive
        # 移动者加载背景图像的锚点及对应坐标
        # 默认图片加载到左上角
    ...
    def set_canvas_image(self, cimg):
        self.set_canvas_index = cimg

    # 获取画布上图片
    def get_canvas_image(self):
        return self.set_canvas_index
    ...
    def update_life_status(self):
        self.lives_num -= 1
        if self.lives_num <= 0:
            self.state = config.life_stauts_dead
        else:
            self.state = config.life_stauts_reset
    ...
    def errors_happened(self):
        if self.state is config.life_stauts_dead:
            self.del_canvas_image(self.bg_image_tag)
        elif self.state is config.life_stauts_reset:
            x = 0
            y = 0
            if self.anchor == tkinter.NW:
            ...
            self.canvas.move(self.bg_image_tag, x, y)
            self.state = config.life_stauts_alive